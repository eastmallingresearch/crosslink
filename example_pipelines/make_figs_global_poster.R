...
plots = matrix(list(p2,p4),nrow=1,byrow=TRUE)
w=unit(c(1,1),"null")
h=unit(c(1),"null")
...
gtab = gtable_add_grob(gtab, textGrob("B", x=0.01, y=0.98, hjust=0,vjust=1,gp=gp), t=1, l=1, b=1, r=2, clip="off",z=101,name="B")
#gtab = gtable_add_grob(gtab, textGrob("C", x=0.01, y=0.45, hjust=0,vjust=1,gp=gp), t=1, l=1, b=2, r=2, clip="off",z=102,name="C")
gtab = gtable_add_grob(gtab, textGrob("D", x=0.51, y=0.98, hjust=0,vjust=1,gp=gp), t=1, l=1, b=1, r=2, clip="off",z=103,name="D")

res=600
width=7.5
height=3.25
ptsize=10
tiff("poster.tiff",res=res,width=width*res,height=height*res,compression="lzw",bg="white",pointsize=ptsize)
grid.draw(gtab)
dev.off()
