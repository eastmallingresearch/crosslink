...
plt = ggplot(dat, aes(x = factor(GIBBS_PROB_UNIDIR), y = hk_score)) +
    ...
    xlab("prob unidirectional mode")
ggsave(file="figs/punidir_vs_hkscore.png",plot=plt,dpi=600)

plt = ggplot(dat, aes(x = factor(GIBBS_PROB_UNIDIR), y = logmap)) +
    geom_boxplot() +
    guides(fill=guide_legend(title="p_unidir"))
ggsave(file="figs/punidir_vs_mappingscore.png",plot=plt,dpi=600)
