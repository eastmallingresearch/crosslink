...
if [ "$(pwd)" != '/home/vicker/crosslink/ploscompbiol_data/simdata/test_mapping' ]
then
...
do
    ...
    GA_USE_MST=$(python -c "import random as r; r.seed(${RANDOM}); print r.choice([0,99])")
    GA_MINLOD=$((RANDOM%10+1))
    GA_MST_NONHK=$(python -c "import random as r; r.seed(${RANDOM}); print r.choice([0,1])")
    GA_OPTIMISE_METH=$(python -c "import random as r; r.seed(${RANDOM}); print r.choice([0,1])")
    vals=( $(python -c "import random as r; r.seed(${RANDOM}); x=r.random(); y=r.random(); z=r.random(); t=x+y+z; x/=t;y/=t;print x,y") )
    GA_PROB_HOP=${vals[0]}
    GA_MAX_HOP=$(python -c "import random as r; r.seed(${RANDOM}); print r.random()")
    GA_PROB_MOVE=${vals[1]}
    GA_MAX_MVSEG=$(python -c "import random as r; r.seed(${RANDOM}); print r.random()")
    GA_MAX_MVDIST=$(python -c "import random as r; r.seed(${RANDOM}); print r.random()")
    GA_PROB_INV=$(python -c "import random as r; r.seed(${RANDOM}); print r.random()")
    GA_MAX_SEG=$(python -c "import random as r; r.seed(${RANDOM}); print r.random()")
