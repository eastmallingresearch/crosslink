...
aov.out = aov(map ~ treatment + Error(SAMPLE_DIR/treatment), data=dat)
summary(aov.out)

#cat("============pairwise t tests with bonferroni correction")
#with(dat, pairwise.t.test(x=map, g=treatment, p.adjust.method="bonf", paired=T))

cat("============2-factor anova")
aov.tbys = aov(map ~ treatment + SAMPLE_DIR, data=dat)
summary(aov.tbys)
