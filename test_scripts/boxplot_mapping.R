...
plt = ggplot(dat, aes(x = factor(GA_MINLOD), y = mapping_score)) +
    geom_boxplot() +
    guides(fill=guide_legend(title="minlod"))
ggsave(file="figs/minlod.png",plot=plt,dpi=600)

plt = ggplot(dat, aes(x = factor(GA_MST_NONHK), y = mapping_score)) +
    geom_boxplot() +
    guides(fill=guide_legend(title="mstnonhk"))
ggsave(file="figs/mstnonhk.png",plot=plt,dpi=600)

plt = ggplot(dat, aes(x = factor(GA_OPTIMISE_METH), y = mapping_score)) +
    geom_boxplot() +
    guides(fill=guide_legend(title="optimisemeth"))
ggsave(file="figs/optimisemeth.png",plot=plt,dpi=600)
